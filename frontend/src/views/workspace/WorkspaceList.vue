    ...
    <main class="main-content">
      ...
      <div class="content">
        ...
        <div v-else-if="workspaces.length === 0" class="empty">
          ...
          <button class="btn-primary" @click="showCreate = true">Workspace erstellen</button>
        </div>

        <div v-else class="ws-grid">
          <div v-for="ws in workspaces" :key="ws.id" class="ws-card" @click="$router.push('/workspaces/' + ws.id)">
            <h4>{{ ws.name }}</h4>
            <p v-if="ws.description" class="ws-desc">{{ ws.description }}</p>
            <div class="ws-meta">
              <span class="ws-badge" v-if="ws.indexing_strategy?.wiki_enabled">📖 Wiki</span>
              <span class="ws-badge" v-if="ws.indexing_strategy?.vector_enabled">🔍 Vector</span>
      ...
      <div v-if="showCreate" class="dialog-overlay" @click.self="showCreate = false">
        <div class="dialog">
          ...
          <div class="dialog-actions">
            ...
            <button class="btn-primary" @click="createWorkspace" :disabled="!newName.trim()">Erstellen</button>
          </div>
...
.header { padding: 1rem 1.5rem; border-bottom: 1px solid var(--color-border); display: flex; justify-content: space-between; align-items: center; }
.content { padding: 1.5rem; }

.ws-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1rem; }
.ws-card { background: white; border: 1px solid var(--color-border); border-radius: 8px; padding: 1.25rem; cursor: pointer; transition: box-shadow 0.2s; }
.ws-card:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
.ws-card h4 { margin-bottom: 0.25rem; }
.ws-desc { color: var(--color-text-secondary); font-size: 0.875rem; margin-bottom: 0.75rem; }
.ws-meta { display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap; }
.ws-badge { font-size: 0.75rem; padding: 0.125rem 0.5rem; background: var(--color-bg-secondary); border-radius: 4px; }
.ws-date { font-size: 0.75rem; color: var(--color-text-secondary); margin-left: auto; }
...
.btn-primary { padding: 0.5rem 1rem; background: var(--color-primary); color: white; border: none; border-radius: 6px; font-size: 0.9rem; }
.btn-primary:disabled { opacity: 0.5; }
.btn-secondary { padding: 0.5rem 1rem; background: var(--color-bg-secondary); border: 1px solid var(--color-border); border-radius: 6px; font-size: 0.9rem; }
