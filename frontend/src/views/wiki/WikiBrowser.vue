    ...
    <main class="wiki-main">
      ...
      <template v-if="workspaceId">
        <div class="wiki-toolbar">
          <input v-model="searchQuery" @input="loadPages" placeholder="Wiki durchsuchen..." class="search-input" />
        ...
        <div class="wiki-content">
          ...
          <div v-else-if="pages.length === 0" class="status-text">
            <p>Noch keine Wiki-Seiten. Generiere Seiten aus Dokumenten oder erstelle sie manuell.</p>
          </div>
          <div v-else class="page-list">
            <div v-for="p in pages" :key="p.id" class="page-card" @click="$router.push(`/wiki/${workspaceId}/${encodeURIComponent(p.slug)}`)">
              <div class="page-type-badge">{{ typeIcon(p.page_type) }}</div>
              <div class="page-info">
                <h4>{{ p.title }}</h4>
                <p class="page-summary">{{ p.summary || 'Keine Zusammenfassung' }}</p>
                <div class="page-meta">
      ...
      <div v-if="showCreate" class="dialog-overlay" @click.self="showCreate = false">
        <div class="dialog">
          ...
          <div class="field"><label>Titel *</label><input v-model="form.title" placeholder="Titel" @input="generateSlug" /></div>
          <div class="field"><label>Slug *</label><input v-model="form.slug" placeholder="wiki/mein-artikel" /></div>
          ...
          <div class="dialog-actions">
            ...
            <button class="btn-primary" @click="createPage" :disabled="!form.title || !form.slug">Erstellen</button>
          </div>
...
async function loadWorkspaces() {
  try { const res = await axios.get("/api/v1/workspaces"); workspaces.value = res.data.workspaces || []; } catch {}
}
...
async function loadPages() {
  ...
  try {
    ...
    const res = await axios.get(`/api/v1/wiki/${workspaceId.value}/pages`, { params });
    pages.value = res.data.pages || [];
    totalPages.value = res.data.total || 0;
  } catch (e: any) { console.error("Failed to load pages", e); }
  finally { loading.value = false; }
}

function generateSlug() {
  if (form.value.title && !form.value.slug.startsWith("wiki/")) {
    form.value.slug = "wiki/" + form.value.title.toLowerCase().replace(/[^a-z0-9äöüß\s-]/g,"").replace(/\s+/g,"-").replace(/-+/g,"-").slice(0,80);
  }
}
...
async function createPage() {
  ...
  try {
    ...
    showCreate.value = false; form.value = { title: "", slug: "", page_type: "article" };
    await loadPages();
  } catch (e: any) { createError.value = e.response?.data?.error || "Fehler"; }
}

function typeIcon(t: string) { return t === "entity" ? "👤" : t === "concept" ? "💡" : "📄"; }
