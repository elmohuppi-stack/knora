...
- [x] **Chat → Wiki-Artikel-Verbund** (`wiki-from-chat.ts`, `WikiClusterReview.vue`): aus einem Gespräch wird ein kompletter Verbund in der üblichen Taxonomie erzeugt — **1 Summary** (langer Übersichtsartikel, Hauptseite) + **n Concepts** (thematische Vertiefungen) + **m Entities** (Personen/Orte/Begriffe, kurz/definitorisch), untereinander per `[[slug|Text]]` verlinkt. Quelle: LLM-Wissen, geformt durchs Gespräch (Zielgruppe/Stil/Deckel im Dialog), optional RAG-gegroundet. Zweistufig (Plan → Artikel), als `status="draft"` mit `page_metadata.cluster_id` gespeichert; das **Transkript wird als Quell-Dokument** (`type="chat"`) angelegt und via `source_document_id` verknüpft. Ablauf: Generierung asynchron (Fortschritt im Activity-Log), Review pollt `GET .../clusters/:id/pages` inkl. **echtem Status** (`getChatWikiStatus` → started/completed/failed; „Veröffentlichen" erst bei `completed` aktiv), Regenerieren = ersetzen, dann `POST .../clusters/:id/publish`. Intra-Cluster-Links greifen, da erst alle Seiten angelegt und dann `resolveLinks`/`updateIncomingLinks` laufen. Entwürfe sind aus Browser/Graph/Statistik ausgeblendet (`status`-Filter); der Wiki-Browser zeigt einen **Banner mit offenen Entwurfs-Verbünden** (`draft-clusters`) zurück ins Review.
- [x] **Chat-Verbesserungen** (Voraussetzung + UX): der Chat sendet dem LLM jetzt den **bisherigen Gesprächsverlauf** (vorher zustandslos, sonst kein iteratives Entwickeln), **weiches Grounding** (nutzt Workspace-Kontext wenn passend, sonst eigenes Wissen statt zu verweigern), `max_tokens` 1024→4096 (keine abgeschnittenen Antworten), mehrzeilige mitwachsende Eingabe, zweiphasiger „denkt nach / schreibt …"-Indikator, Sessions löschbar (`DELETE /chat/sessions/:id`)
